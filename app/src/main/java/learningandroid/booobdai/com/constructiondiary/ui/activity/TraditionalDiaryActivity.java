...
import learningandroid.booobdai.com.constructiondiary.dao.DiaryDbManager;
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        mMenuItem.setOnMenuItemClickListener(new MenuItem.OnMenuItemClickListener() {//保存
            ...
            public boolean onMenuItemClick(MenuItem item) {
                showToast("保存点击了");
                initDiaryBean(diaryBean);
                if (isNeedCreatedDb) {
                    Logger.e("");
                    getDiaryDao().insert(diaryBean);

//                    isNeedCreatedDb=false;
                } else {
                    getDiaryDao().update(diaryBean);
                }
...
//                DiaryBean diaryBean = diaryBeanDao.queryBuilder().where(DiaryBeanDao.Properties.RecordTime.eq("2017年3月10日")).build().unique();
    ...
    //
    private void initDiaryBean(DiaryBean diaryBean) {
        if (diaryBean == null) {
...
//        diaryBeanDao = GreenDaoManager.getInstance().getSession().getDiaryBeanDao();
    ...
    private DiaryDbManager getDiaryDao() {
        return new DiaryDbManager();
    }
}
